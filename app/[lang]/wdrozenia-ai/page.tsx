...
export async function generateMetadata({
  ...
  const c = (await getDictionary(lang)).aiIntegration;
...
export default async function Page({
  ...
  const { lang: rawLang } = await params;
  const lang = await resolveLang(rawLang);
  const content = (await getDictionary(lang)).aiIntegration as LandingContent;
  return (
    <SeoLanding
      lang={lang}
      content={content}
      ...
      parent={null}
      primaryHref="#kontakt"
      secondaryHref="/oprogramowanie-dedykowane"
      bannerHref="#kontakt"
    />
  );
}
